...
def load_proof_steps(fpath, lm=None, _all=False, graph=False):
    ...
    for ps in proof_steps:
        # Check if the context_label is valid
        if ps.context_label not in lm.database.propositions:
            print(f"Missing key in database.propositions: {ps.context_label}")
            continue  # Skip invalid proof steps
        
        # Check if the prop_label is valid
        if ps.prop_label not in lm.database.propositions:
            print(f"Missing key in database.propositions: {ps.prop_label}")
            continue  # Skip invalid proof steps
        
        # Assign context and prop to the proof step
        ps.context = lm.database.propositions[ps.context_label]
        ps.prop = lm.database.propositions[ps.prop_label]
        valid_proof_steps.append(ps)
    ...
    if _all:
        ...
        if graph:
            ...
            with open(fpath + '_encode', 'rb') as f:
                inputs = pickle.load(f)
        return valid_proof_steps, inputs, props
    
    # Return valid proof steps only
    return valid_proof_steps
...
def load_gen_proof_steps(fpath, lm=None, _all=False):
    ...
    steps = []
    for ps in proof_steps:
        # Check for missing context_label
        if ps.context_label not in lm.database.propositions:
            print(f"Missing key in database.propositions (context_label): {ps.context_label}")
            continue  # Skip this proof step
        
        ps.context = lm.database.propositions[ps.context_label]
        
        # Check for missing prop_label
        if ps.prop_label not in lm.database.propositions:
            print(f"Missing key in database.propositions (prop_label): {ps.prop_label}")
            continue  # Skip this proof step

        ps.prop = lm.database.propositions[ps.prop_label]

        # Only add proof steps with unconstrained arity > 0
        if ps.prop.unconstrained_arity() > 0:
            steps.append(ps)
...
def encode_proof_step(statement, f, hyps, lm, config):
    ...
    # Dynamically add missing tokens to config.encode
    for token in in_string:
        if token not in config.encode:
            print(f"Adding missing token to config.encode: {token}")
            config.encode[token] = len(config.encode)
...
def encode_gen_step(proof_step, lm, config):
    ...
    print(f"Unconstrained variables: {unconstrained_variables}")
    ...
    if fit is None:
        print(f"Warning: 'fit' is None for proof step: {proof_step}")
        return None
    ...
    for i, graph in enumerate(out_graph_structure):
        if i >= len(uv_pos):
            print(f"IndexError: Out graph structure index {i} exceeds UV_Pos length {len(uv_pos)}")
            continue
        string, structured_data = merge_graph_structures_new([graph])

        # Dynamically add missing tokens for output strings
        for token in string:
            if token not in config.encode:
                print(f"Adding missing token to config.encode: {token}")
                config.encode[token] = len(config.encode)
